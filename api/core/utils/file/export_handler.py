...
def query_to_csv(self, job_id: str, fieldnames: List[str], query):
    ...
    job.save()
    csvfile = open(f"{job.job_id}.csv", "w", newline="")

    csvwriter = csv.DictWriter(csvfile, fieldnames=fieldnames)
    csvwriter.writeheader()

    for row in query:
        job = JobModel.objects.get(task_id=self.request.id)

        if job.job_status == JobModel.STARTED or job.job_status == JobModel.RESUMED:
            _dict = model_to_dict(row)
            csvwriter.writerow(_dict)

        if job.job_status == JobModel.REVOKED:
            csvfile.close()
            os.unlink("{}.csv".format(job.task_id))
